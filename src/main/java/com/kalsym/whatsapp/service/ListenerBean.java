            ...
            map.forEach((RequestMappingInfo requestMappingInfo, HandlerMethod handlerMethod) -> {
                try {
                    if (!handlerMethod.getMethod().getName().equalsIgnoreCase("error")
                        ...
                        Logger.application.info(Logger.pattern, WhatsappWrapperServiceApplication.VERSION, "", "name: " + requestMappingInfo.getName());
                        Logger.application.info(Logger.pattern, WhatsappWrapperServiceApplication.VERSION, "", "method: " + handlerMethod.getMethod().getName());
                        Logger.application.info(Logger.pattern, WhatsappWrapperServiceApplication.VERSION, "", "description: " + requestMappingInfo.toString());
                        ...
                        authority.setServiceId("whatsapp-java-service");

                        if (null != authority.getId()) {
                            authorities.add(authority);
                        }
                        Logger.application.info(Logger.pattern, WhatsappWrapperServiceApplication.VERSION, "", "inserted authority", "");

                    }

                } catch (Exception e) {
                    Logger.application.warn(Logger.pattern, WhatsappWrapperServiceApplication.VERSION, "error inserting authority", e.getMessage());
                }

            });

            ResponseEntity<String> response = restTemplate.postForEntity(userServiceUrl, authorities, String.class);
            Logger.application.info(Logger.pattern, WhatsappWrapperServiceApplication.VERSION, "response: " + response);
